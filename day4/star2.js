...
while (
  boardsArr.length > 0 &&
  !losingBoard &&
  currentDrawIndex < drawData.length
) {
  currentDrawIndex++;
  ...
  if (boardsToRemove.length > 0) {
    if (boardsArr.length > 1) {
      boardsArr = boardsArr.filter((_, i) => !boardsToRemove.includes(i));
    } else {
      losingBoard = boardsArr[0];
    }
  }
}

console.log(losingBoard);
const losingDrawnNumber = drawData[currentDrawIndex];
console.log(getScore(losingBoard, losingDrawnNumber));
